...
\begin{table}[h]
    ...
    \begin{tabular}{l r r}
        ...
        \midrule
        AEC/Arts digital twins~\cite{marketsandmarkets_digital_twin_2025} & \texteuro149.8B & 47.9\% \\
        AR/VR~\cite{vmr_ar_vr_training} & \texteuro21.7B & 30.2\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
\paragraph{Foreground IP strategy.} The foreground IP, consisting of domain-specific adaptations of the scene completion pipeline for each target market, will be managed in collaboration with LRD on a per-application basis. Protectable outputs include domain-specific adapters (e.g., the reuse-passport generation workflow), trained model weights for specific object categories, and proprietary datasets generated during future validation campaigns.
...
\todo{Add information about YouTube channel and content creation activities (90k+ subscribers, 7M+ views) as an additional dissemination channel, if this is to be included in the thesis.}
